...
def ex_dir(songs):
    ...
    if not os.path.isdir(dir_):
        ...
        sys.exit(0)
    ex_dir_subdirs(dir_, songs)

def ex_dir_subdirs(dir_, songs):
    subdirs = [os.path.join(dir_,subdir) for subdir in os.listdir(dir_)] 
    for d in subdirs:
            if (os.path.isdir(d)):
                print(f'Dir: {d}')
                ex_dir_subdirs(d,songs)
            else:
                ex_dir_files(dir_,songs)
                break;
    
def ex_dir_files(dir_, songs):
